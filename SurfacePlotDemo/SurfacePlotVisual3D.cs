...
namespace SurfacePlotDemo
{
    ...
    public class SurfacePlotVisual3D : ModelVisual3D
    {
        ...
        public void timer_Tick(object sender, EventArgs e)
        {
            ...
            if (Global_variable.record == true)
            {
                Global_variable.ti++;
                //    SphereShow(Global_variable.tempbox[0], Global_variable.tempbox[1], Global_variable.tempbox[2]);
                //}
                //Global_variable.tempbox.Clear();
                //Global_variable.record = false;

                foreach (Cluster cl in Global_variable.ClusterGlobalList)
                {
                    if (cl.ClusterIndiividual.Count > Global_variable.ti && Global_variable.ti < 30)
                        SphereShow(cl.ClusterIndiividual[Global_variable.ti].decimal_value[0], cl.ClusterIndiividual[Global_variable.ti].decimal_value[1], cl.ClusterIndiividual[Global_variable.ti].result_function);

                }

                if (Global_variable.ti==30)
                {
                    Global_variable.record = false;
                }
            }
        ...
        {
           if (Global_variable.surface==false)
           {
               plotModel.Children.Clear();
               Global_variable.surface = true;
           }
        ...
        private Model3D CreateModel()
        {
            ...
            //var cube = new MeshBuilder(true, true);
            //cube.AddBox(new Point3D(0, 0, 0), 0.005, 0.005, 0.005, MeshBuilder.BoxFaces.All);
            //var CubeModel = new GeometryModel3D(cube.ToMesh(), Materials.Blue);

            //plotModel.Children.Add(CubeModel);
            plotModel.Children.Add(surfaceModel);
            Global_variable.kol_elem++;
        ...
        public  Model3D SphereShow(double x, double y, double z)
        {
            Random rnd = new Random();
            var cube = new MeshBuilder(true, true);
            cube.AddBox(new Point3D(x,y,z), 0.08, 0.08, 0.08, MeshBuilder.BoxFaces.All);

            if (Global_variable.find_max_or_min == 1)
            {
                var CubeModel = new GeometryModel3D(cube.ToMesh(), Materials.Red);

                //Action action = () => { plotModel.Children.Add(CubeModel); };
